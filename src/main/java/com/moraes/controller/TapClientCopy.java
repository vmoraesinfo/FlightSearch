    ...
    private Tap[] extractBestPrices(Map<String, Object> response) {
        if(response != null) {       
            ...
            if(data != null){
                ...
                Object bestPricesObject = data.get("bestPriceForDates");
                if(bestPricesObject instanceof Map){
                    Map<?,?> bestPrices = (Map<?,?>) bestPricesObject;
                    bestPriceForDates = mapper.convertValue(bestPrices.values(), Tap[].class);
                }
